...
var mockResult = [result0, result1];

chellSearch.factory('SearchAdapter', function ($http, $q, _) {
    return {
...
var externalToSearchContent = function(externalContent) {
    var searchContent = externalContent; //replace with mapping
    return searchContent;
};
var searchToExternalContent = function(searchContent) {
    var externalContent = searchContent; //replace with mapping
    return externalContent;
};

chellSearch.run(function($httpBackend, $base64) {
    ...
    $httpBackend.whenGET(/search\/result/).respond(function(method, url, data, headers) {
        return authenticated(headers) ? [200, mockResult] : [401];
    });
});
